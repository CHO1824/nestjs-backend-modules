...
import { InvalidTransactionStatusTransitionError, TransactionNotFoundError } from "../errors/transaction.error";
...
export class TransactionService {
  ...
  async findMany(userId: string, query: TransactionListQueryDto): Promise<TransactionListResponseDto> {
    ...
    return {
      ...
      nextCursor:
        hasNext && last ? encodeTransactionListCursor({ createdAt: last.createdAt.toISOString(), id: last.id }) : null,
      hasNext,
      limit: query.limit,
    };
  }
  ...
  async cancelByUser(transactionId: string, userId: string, reason?: string): Promise<TransactionDetailResponseDto> {
