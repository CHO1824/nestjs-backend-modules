...
export class EmailSender implements NotificationSender {
    ...
    if (!apiKey || !domain || !fromEmail) {
      ...
      if (isProduction) {
        const missing = [
          ...
          .join(", ");
        throw new Error(
          `[FATAL] Mailgun is not configured in production. Missing: ${missing}.`,
        );
      }
  ...
  async send(outboxId: string, recipientInfo: string | string[], content: RenderedContent): Promise<void> {
    ...
    if (!this.configured || !this.client || !this.domain || !this.fromAddress) {
      this.logger.log(`[MOCK EMAIL] Outbox ID: ${outboxId} - Send to ${recipientInfo} skipped (Mailgun not configured)`);
      return;
    }
