...
const TARGET_RESOLVERS: Record<string, (user: UserWithContext) => string[]> = {
  ...
  SMS: (user) =>
    user.phoneNumber && user.phoneCountryCode ? [`${user.phoneCountryCode}${user.phoneNumber.replace(/^0/, "")}`] : [],
  KAKAO: (user) =>
    user.phoneNumber && user.phoneCountryCode ? [`${user.phoneCountryCode}${user.phoneNumber.replace(/^0/, "")}`] : [],
};
...
export class NotificationService {
  ...
  async publish(userId: string, dto: CreateNotificationDto, traceId?: string, tx?: Prisma.TransactionClient) {
    ...
    const uiData = this.templateService.render(eventType, typedPayload, user.locale);
