  ...
  private async processJob(job: OutboxJob) {
    try {
      ...
      if (job.channel === "KAKAO" || job.channel === "SMS") {
        // KAKAO uses code defaults (NCP template match); SMS uses the DB override.
        const content = await this.templateService.render(eventType, typedPayload, job.locale, job.channel);
      ...
      const content: RenderedContent = await this.templateService.render(
        eventType,
        typedPayload,
        job.locale,
        job.channel,
